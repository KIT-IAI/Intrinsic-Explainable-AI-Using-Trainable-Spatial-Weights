...
class ERA5Dataset(BaseDataset):
    ...
    def __init__(self,
        ...
        """ ERA5 weather dataset without any data transformations.
        ...
        with open(weather_stats_path, 'r') as stats_json:
            self.statistics = json.load(stats_json)

        # load weather dataset from multiple netcad files
        ds = xr.open_mfdataset(
            self.data_path,
            engine='h5netcdf',
            chunks=dict(time=2 * self.weather_horizon)
        )

        # filter by time
        timesteps = pd.date_range(time[0], time[-1] + pd.Timedelta(f'{weather_horizon + 1}h'), freq='1h')
    ...
    def normalize_weather(self, weather, weather_var):
        statistics = self.statistics[weather_var]
    ...
    def __getitem__(self, index) -> np.ndarray:
        timesteps = pd.date_range(self.time[index] + pd.Timedelta('1h'),
                                  self.time[index] + pd.Timedelta(f'{self.weather_horizon}h'),
                                  freq='1h')
        weather = self.ds.sel(time=timesteps)
